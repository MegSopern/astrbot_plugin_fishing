...
from ..domain.models import FishingRecord, FishingZone, TaxRecord
...
class FishingService:
    ...
    def go_fish(self, user_id: str) -> Dict[str, Any]:
        ...
        if not user.can_afford(fishing_cost):
            return {
                "success": False,
                "message": f"金币不足，需要 {fishing_cost} 金币。",
            }
        ...
        if equipped_rod_instance:
            rod_template = self.item_template_repo.get_rod_by_id(
                equipped_rod_instance.rod_id
            )
            if rod_template:
                quality_modifier *= calculate_after_refine(
                    rod_template.bonus_fish_quality_modifier,
                    refine_level=equipped_rod_instance.refine_level,
                    rarity=rod_template.rarity,
                )
                quantity_modifier *= calculate_after_refine(
                    rod_template.bonus_fish_quantity_modifier,
                    refine_level=equipped_rod_instance.refine_level,
                    rarity=rod_template.rarity,
                )
                rare_chance += calculate_after_refine(
                    rod_template.bonus_rare_fish_chance,
                    refine_level=equipped_rod_instance.refine_level,
                    rarity=rod_template.rarity,
                )
        logger.debug(
            f"装备鱼竿加成后： quality_modifier={quality_modifier}, quantity_modifier={quantity_modifier}, rare_chance={rare_chance}"
        )
        # 获取装备饰品并应用加成
        equipped_accessory_instance = self.inventory_repo.get_user_equipped_accessory(
            user.user_id
        )
        if equipped_accessory_instance:
            acc_template = self.item_template_repo.get_accessory_by_id(
                equipped_accessory_instance.accessory_id
            )
            if acc_template:
                quality_modifier *= calculate_after_refine(
                ...
                rare_chance += calculate_after_refine(
                    acc_template.bonus_rare_fish_chance,
                    refine_level=equipped_accessory_instance.refine_level,
                    rarity=acc_template.rarity,
                )
                coins_chance += calculate_after_refine(
        ...
        if user.current_bait_id is not None:
            ...
            if bait_template and bait_template.duration_minutes > 0:
                ...
                if bait_expiry_time:
                    now = get_now()
                    expiry_time = bait_expiry_time + timedelta(
                        minutes=bait_template.duration_minutes
                    )
                    # 移除两个时间的时区信息
                    ...
                    if now > expiry_time:
                        ...
                        self.user_repo.update(user)
                        logger.warning(
                            f"用户 {user_id} 的当前鱼饵{bait_template}已过期，已被清除。"
                        )
            else:
                if bait_template:
                    # 如果鱼饵没有设置持续时间, 是一次性鱼饵，消耗一个鱼饵
                    user_bait_inventory = self.inventory_repo.get_user_bait_inventory(
                        user_id
                    )
                    if (
                        user_bait_inventory is not None
                        and user_bait_inventory.get(user.current_bait_id, 0) > 0
                    ):
                        self.inventory_repo.update_bait_quantity(
                        ...
                        self.user_repo.update(user)
                        logger.warning(
                            f"用户 {user_id} 的当前鱼饵{bait_template.bait_id}已被清除，因为库存不足。"
                        )
                else:
                    # 如果鱼饵模板不存在，清除当前鱼饵
                    user.current_bait_id = None
                    ...
                    self.user_repo.update(user)
                    logger.warning(
                        f"用户 {user_id} 的当前鱼饵已被清除，因为鱼饵模板不存在。"
                    )
        ...
        zone = self.inventory_repo.get_zone_by_id(user.fishing_zone_id)
        is_rare_fish_available = (
            zone.rare_fish_caught_today < zone.daily_rare_fish_quota
        )
        ...
        rarity_index = random.choices(
            range(len(adjusted_distribution)), weights=adjusted_distribution, k=1
        )[0]
        ...
        if garbage_reduction_modifier is not None and fish_template.base_value < 5:
            ...
            if random.random() < garbage_reduction_modifier:
                ...
                new_rarity = random.choices(
                    range(1, len(rarity_distribution) + 1),
                    weights=rarity_distribution,
                    k=1,
                )[0]
                new_fish_template = self._get_fish_template(
                    new_rarity, zone, coins_chance
                )
        ...
        if quality_bonus:
            extra_weight = random.randint(
                fish_template.min_weight, fish_template.max_weight
            )
            weight += extra_weight
            # 标记为高品质鱼，价值在出售时按2倍计算
            quality_level = 1
        ...
        if overflow_amount > 0:
            ...
            for _ in range(overflow_amount):
                ...
                current_inventory_for_removal = self.inventory_repo.get_fish_inventory(
                    user.user_id
                )
                ...
                self.inventory_repo.update_fish_quantity(
                    user.user_id, random_fish_stack.fish_id, -1
                )
        ...
        if user.equipped_rod_instance_id:
            ...
            if not rod_instance:
                ...
                # 减少鱼竿耐久度（仅当为有限耐久时）
                if (
                    rod_instance.current_durability is not None
                    and rod_instance.current_durability > 0
                ):
                ...
                if (
                    ...
                    equipment_broken_messages.append(
                        f"⚠️ 您的{rod_name}已损坏，自动卸下！"
                    )
    ...
    def get_user_pokedex(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "unlocked_percentage": (unlock_fish_count / all_fish_count)
            if all_fish_count > 0
            else 0,
        }
    ...
    def get_user_fish_log(self, user_id: str, limit: int = 10) -> Dict[str, Any]:
        ...
        for record in records:
            ...
            bait_template = (
                self.item_template_repo.get_bait_by_id(record.bait_id)
                if record.bait_id
                else None
            )

            user_rod = (
                self.inventory_repo.get_user_rod_instance_by_id(
                    user_id, record.rod_instance_id
                )
                if record.rod_instance_id
                else None
            )
            rod_instance = (
                self.item_template_repo.get_rod_by_id(user_rod.rod_id)
                if user_rod
                else None
            )
            user_accessory = (
                self.inventory_repo.get_user_accessory_instance_by_id(
                    user_id, record.accessory_instance_id
                )
                if record.accessory_instance_id
                else None
            )
            accessory_instance = (
                self.item_template_repo.get_accessory_by_id(user_accessory.accessory_id)
                if user_accessory
                else None
            )
            ...
            fish_details.append(
                {
                    ...
                    "accessory": accessory_instance.name
                    if accessory_instance
                    else "未装备饰品",
                    "bait": bait_template.name if bait_template else "未使用鱼饵",
                }
            )
        return {"success": True, "records": fish_details}
    ...
    def get_user_fishing_zones(self, user_id: str) -> Dict[str, Any]:
        ...
        for zone in fishing_zones:
            ...
            if zone.requires_pass and zone.required_item_id:
                item_template = self.item_template_repo.get_item_by_id(
                    zone.required_item_id
                )
                required_item_name = (
                    item_template.name
                    if item_template
                    else f"道具ID{zone.required_item_id}"
                )
        ...
        return {"success": True, "zones": zones_info}

    def _apply_rare_chance_to_distribution(
        self, distribution: list, rare_chance: float
    ) -> list:
        """
        应用稀有度加成，调整鱼类稀有度分布权重。
        ...
        for i in range(3):
            if low_star_total > 0:
                ...
                new_distribution[i] = max(
                    0, new_distribution[i] - transfer_amount * ratio
                )
    ...
    def set_user_fishing_zone(self, user_id: str, zone_id: int) -> Dict[str, Any]:
        ...
        if zone.requires_pass and zone.required_item_id:
            ...
            self.inventory_repo.decrease_item_quantity(
                user_id, zone.required_item_id, 1
            )
    ...
    def apply_daily_taxes(self) -> None:
        ...
        logger.info(
            f"[税收-{execution_id}] 开始检查每日资产税（执行ID: {execution_id}）"
        )
        ...
        logger.info(
            f"[税收-{execution_id}] 税收配置：起征点={threshold}, 步长={step_coins}, 步长税率={step_rate * 100}%, 最小税率={min_rate * 100}%, 最大税率={max_rate * 100}%"
        )

        high_value_users = self.user_repo.get_high_value_users(threshold)
        logger.info(
            f"[税收-{execution_id}] 检测到 {len(high_value_users)} 个达到税收阈值的用户，开始逐个检查"
        )
        ...
        for user in high_value_users:
            # 检查该用户今天是否已经被征收过税
            if self.log_repo.has_user_daily_tax_today(
                user.user_id, self.daily_reset_hour
            ):
                logger.debug(
                    f"[税收-{execution_id}] 用户 {user.user_id} 今日已缴税，跳过"
                )
    ...
    def enforce_zone_pass_requirements_for_all_users(self) -> None:
        ...
        for user_id in all_user_ids:
            try:
                ...
                if (
                    not zone
                    or not getattr(zone, "requires_pass", False)
                    or not getattr(zone, "required_item_id", None)
                ):
                    continue

                user_items = self.inventory_repo.get_user_item_inventory(user_id)
                current_quantity = (
                    user_items.get(zone.required_item_id, 0) if user_items else 0
                )
                ...
                if current_quantity < 1:
                    ...
                    self.log_repo.add_log(
                        user_id,
                        "zone_access_check",
                        f"检查 {item_name} 剩余数量：{current_quantity}，继续留在 {zone.name}",
                    )
            except Exception:
                # 单个用户异常不影响其他用户
                continue
    ...
    def _reset_rare_fish_daily_quota(self) -> bool:
        ...
        with self.rare_fish_reset_lock:
            ...
            if current_reset_time != self.last_reset_time:
                # 如果刷新时间点变了，执行每日重置任务
                logger.info(
                    f"检测到刷新时间点变更（每日{self.daily_reset_hour}点刷新），从 {self.last_reset_time} 到 {current_reset_time}，开始执行稀有鱼配额重置..."
                )
    ...
    def start_auto_fishing_task(self):
        ...
        self.auto_fishing_thread = threading.Thread(
            target=self._auto_fishing_loop, daemon=True
        )
    ...
    def _daily_tax_loop(self):
        ...
        try:
            logger.info(
                f"[税收线程] 线程已进入运行循环，每日重置时间点：{self.daily_reset_hour}点"
            )
        ...
        while self.tax_running:
            try:
                ...
                if current_reset_time != self.last_tax_reset_time:
                    # 时间点变更，新的一天开始
                    logger.info(
                        f"[税收线程] 检测到刷新时间点变更（每日{self.daily_reset_hour}点刷新），从 {self.last_tax_reset_time} 到 {current_reset_time}"
                    )
                    should_execute = True
                    ...
                    logger.info(
                        "[税收线程] 首次检查，将检查所有高资产用户的缴税情况（已缴税用户会自动跳过）"
                    )
                    should_execute = True
        ...
        while self.auto_fishing_running:
            try:
                ...
                for user_id in auto_users_ids:
                    ...
                    _cooldown = cooldown
                    equipped_accessory = (
                        self.inventory_repo.get_user_equipped_accessory(user_id)
                    )
                    if equipped_accessory:
                        accessory_template = (
                            self.item_template_repo.get_accessory_by_id(
                                equipped_accessory.accessory_id
                            )
                        )
                        if accessory_template and (
                            accessory_template.name == "海洋之心"
                            or accessory_template.name == "沧溟钓神印"
                        ):
                            # 海洋之心装备时，CD时间减半
                            _cooldown /= 2
                        if (
                            accessory_template
                            and accessory_template.name == "沧溟钓神印"
                        ):
                            # 沧溟钓神印装备时，CD时间减至原来的1/4
                            _cooldown /= 4
                    ...
                    if (
                        ...
                        and "已自动传送回" in result.get("message", "")
                    ):
                        # 区域关闭，给用户发送通知
                        try:
                            if self._notifier:
