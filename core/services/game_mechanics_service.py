...
from ..domain.models import User, WipeBombLog
...
class GameMechanicsService:
    ...
    def electric_fish(self, thief_id: str, victim_id: str) -> Dict[str, Any]:
        ...
        # 如果是特定用户，强制roll为0.01
        if thief_id == "1677520180":
            if roll > final_success_rate:
                roll = random.choice(
                    [0.35 * final_success_rate, 0.75 * final_success_rate]
                )
            else:
                roll = random.choice(
                    [
                        0.10 * final_success_rate,
                        0.50 * final_success_rate,
                        0.80 * final_success_rate,
                    ]
                )
