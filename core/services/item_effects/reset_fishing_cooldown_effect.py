...
class ResetFishingCooldownEffect(AbstractItemEffect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.fishing_service:
            raise ValueError(
                "ResetFishingCooldownEffect requires fishing_service dependency"
            )
    ...
    def apply(
        ...
        if quantity > 999:
            return {
                "success": False,
                "message": f"【{item_template.name}】一次最多只能使用 999 个。",
            }
        ...
        for _ in range(quantity):
            ...
            if result and result.get("success"):
                ...
                # 收集装备损坏消息
                if "equipment_broken_messages" in result:
                    equipment_broken_messages.extend(
                        result["equipment_broken_messages"]
                    )
            else:
                fail_count += 1
