...
from ..utils import parse_target_user_id, safe_datetime_handler, to_percentage
    ...
    if len(args) >= 3:
        if args[2].isdigit():
            ...
            if times <= 0:
                ...
                return
            if times > 1000:
                yield event.plain_result("❌ 单次最多只能进行1000次十连抽卡")
                return
        else:
...
async def start_wheel_of_fate(self: "FishingPlugin", event: AstrMessageEvent):
    ...
    if len(args) < 2:
        ...
        max_fee = config.get("max_entry_fee", 9999999)
